...
#define BORDER 5
#define TITLE_SHADOW_OFFSET   CGSizeMake(0, 0)
...
- (id)initWithTitle:(NSString *)title 
{
    if ((self = [super init]))
    {
        self.frame = CGRectMake(0,20, 80, 40);
        ...
        self.titleShadowColor = [UIColor magentaColor];
        ...
        popoverController.popoverBaseColor = [UIColor blueColor];
...
{
    NSUInteger i = 1;
    NSUInteger buttonHeight = BUTTON_HEIGHT -5;
    ...
    for (NSArray *button in buttonsMutableArray)
    {
        ...
        button.frame = CGRectMake(BORDER, buttonY/2, self.bounds.size.width-BORDER*2, buttonHeight/2);
...
-(UIImage*)buttonImage:(UIColor*)color borderWidth:(NSUInteger)borderWidth borderColor:(UIColor*)borderColor
{
    ...
    //Size
    float buttonWidth = self.frame.size.width - 50; //- (BORDER*2);
    ...
    UIBezierPath* roundedRectanglePath = [UIBezierPath bezierPathWithRoundedRect: CGRectMake(0, 0, buttonWidth, 0) cornerRadius: self.cornerRadius];
    [color setFill];
    ...
    if(borderWidth >0)
    {
        if(!borderColor) borderColor = [UIColor blueColor];
